...
for i in range(0,601):
    for j in range(0,251):
        ...
        # Hexagon
        if int((-15/26)*i + 2825/13) <= j <= int((15/26)*i + 425/13) and int(230) <= i <= int(370):
            if int((15/26)*i - 1675/13) <= j <= int((-15/26)*i + 4925/13):
                obstacles.append((i, j))

        # Triangle
        if int((2*i - 906)) <= j <= int(-2*i + 1156) and i >= 455 and 20 <= j <= 230:
            obstacles.append((i, j))
...
pygame.init()
video = vidmaker.Video("djiktra_aaqib_barodawala.mp4", late_export=True)
size = [600, 250]
...
while not done:
    ...
    # Walls
    pygame.draw.rect(monitor, "red", [0, 0, 5, 250], 0)
    pygame.draw.rect(monitor, "red", [0, 0, 600, 5], 0)
    pygame.draw.rect(monitor, "red", [0, 245, 600, 5], 0)
    pygame.draw.rect(monitor, "red", [595, 0, 5, 250], 0)

    # Rectangles
    x, y = rec_pygame([95, 0], 250, 105)
    pygame.draw.rect(monitor, "red", [x, y, 60, 105], 0)

    x, y = rec_pygame([95, 145], 250, 105)
    pygame.draw.rect(monitor, "red", [x, y, 60, 105], 0)
    ...
    pygame.draw.polygon(monitor, "orange", ((235, 87.5), (300, 50),(365, 87.5), (365, 162.5), (300, 200), (235, 162.5)))

    for j in visited_nodes:
        pygame.draw.circle(monitor, (50, 150, 140), to_pygame(j, 250), 1)
        video.update(pygame.surfarray.pixels3d(monitor).swapaxes(0, 1), inverted=False)
        pygame.display.flip()
        clock.tick(500)

    for i in path:
        pygame.draw.circle(monitor, (255, 255, 255), to_pygame(i, 250), 1)
        video.update(pygame.surfarray.pixels3d(monitor).swapaxes(0, 1), inverted=False)
        pygame.display.flip()
        clock.tick(500)
